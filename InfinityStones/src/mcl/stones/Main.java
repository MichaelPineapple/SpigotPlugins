...
public class Main extends JavaPlugin implements Listener
{
    ...
    final PotionEffect timeEffect1 = new PotionEffect(PotionEffectType.SLOWNESS, TIME_FREEZE_DURATION, 100000);
    final PotionEffect timeEffect2 = new PotionEffect(PotionEffectType.MINING_FATIGUE, TIME_FREEZE_DURATION, 10000);
    ...
    public void onEntityDamage(EntityDamageByEntityEvent event)
    {
        if (event.getDamager() instanceof Player)
        {
            ...
            {
                if (item.getItemMeta().getItemName().equals(MIND_TAG))
                {
                    mindLeft(target, p);
    ...
    public void onPlayerInteractEntity(PlayerInteractEntityEvent event)
    {
        if (event.getHand() == EquipmentSlot.HAND)
        {
            ...
            if (item.getType() != Material.AIR)
            {
                String itemTag = item.getItemMeta().getItemName();
    ...
    public void onPlayerInteract(PlayerInteractEvent event)
    {
        if (event.getItem() != null)
        {
            if (event.getHand() == EquipmentSlot.HAND)
            {
                ...
                String itemTag = event.getItem().getItemMeta().getItemName();
    ...
    void smallSplash(Player p, String txt)
    {
        ...
        if (forceGrabTask == -1)
        {
            ...
            forceGrabTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(this, new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    p.sendMessage(ChatColor.YELLOW + "" + countdown);
                    countdown--;
                }
            }, 0, 1);
    ...
    void realityLeft(Player p)
    {
        ...
        w.spawnParticle(Particle.CRIT, targetLoc, 1000);
    }
    ...
    void powerRight(Player p)
    {
        ...
        if (boots != null && !boots.getItemMeta().getItemName().equals(ThanosBoots.getItemMeta().getItemName())) w.dropItemNaturally(ploc, boots);
        if (pants != null && !pants.getItemMeta().getItemName().equals(ThanosPants.getItemMeta().getItemName())) w.dropItemNaturally(ploc, pants);
        if (chest != null && !chest.getItemMeta().getItemName().equals(ThanosChest.getItemMeta().getItemName())) w.dropItemNaturally(ploc, chest);
        if (helmet != null && !helmet.getItemMeta().getItemName().equals(ThanosHelmet.getItemMeta().getItemName())) w.dropItemNaturally(ploc, helmet);
    ...
    {
        target.remove();
        target.getWorld().spawnParticle(Particle.SMOKE, target.getLocation(), 1000, target.getWidth()/2, target.getHeight()/2, target.getWidth()/2, 0);
    }
    ...
    void createItems()
    {
        createStone(SpaceStone, "Space", ChatColor.AQUA, SPACE_TAG, "Anti-Gravity", "Teleport");
        createStone(TimeStone, "Time", ChatColor.GREEN, TIME_TAG, "Freeze Time", "Toggle Day/Night");
        createStone(MindStone, "Mind", ChatColor.YELLOW, MIND_TAG, "Force Grab", "Open Player's Inventory");
        createStone(RealityStone, "Reality", ChatColor.RED, REALITY_TAG, "Summon Minions", "Distort Reality");
        createStone(PowerStone, "Power", ChatColor.LIGHT_PURPLE, POWER_TAG, "Stormbreaker", "Armor");
        createStone(SoulStone, "Soul", ChatColor.GOLD, SOUL_TAG, "Snap (All Stones Required)", "Destroy Soul");
        createArmorPiece(ThanosHelmet, "Helmet");
        createArmorPiece(ThanosChest, "Chestplate");
        createArmorPiece(ThanosPants, "Leggings");
        createArmorPiece(ThanosBoots, "Boots");
    }

    static final ChatColor white = ChatColor.WHITE;
    private void createStone(ItemStack stack, String name, ChatColor color, String tag, String leftLore, String rightLore)
    {
        ChatColor hue = ChatColor.AQUA;
        ChatColor white = ChatColor.WHITE;
        ItemMeta meta = stack.getItemMeta();
        meta.setItemName(tag);
        meta.setDisplayName(hue + name + " Stone");
        meta.addEnchant(Enchantment.INFINITY, 1, true);
        meta.setLore(Arrays.asList(hue+"Left Click: "+white+leftLore, hue+"Right Click: "+white+rightLore));
        stack.setItemMeta(meta);
    }
    private void createArmorPiece(ItemStack stack, String name)
    {
        ChatColor color = ChatColor.DARK_PURPLE;
        ItemMeta meta = stack.getItemMeta();
        meta.setDisplayName(color+"Thanos "+name);
        meta.setItemName("thanos"+name.toLowerCase());
        meta.addEnchant(Enchantment.BLAST_PROTECTION, 100, true);
